...
class RunModelOnData:
    ...
    def __init__(self, args):
        ...
        self.models = {}
        # Set up 3 FastSurferVINN models
    ...
    def get_prediction(
        ...
        print(f"Shape of pred_prob array shape {shape}")


        pred_prob = torch.zeros(shape, **kwargs)

        # inference and view aggregation
        # 3 instances of FastSurferVNN are run and we extract a prediction probability for each iteration
        for plane, model in self.models.items():
            # if plane == os.environ['BRAIN_VIEW']:
            # os.environ['BRAIN_VIEW'] = plane
            LOGGER.info(f"Run {plane} prediction")
            self.set_model(plane)
            # pred_prob is updated inplace to conserve memory
            # Calls Inference.run() which calls Inference.eval()
        ...
        print('done map_label2aparc_aseg')
        ...
        print('done split_cortex_labels')
        return pred_classes
...
if __name__ == "__main__":
    ...
    for subject, (orig_img, data_array) in iter_subjects:
        ...
        try:
            ...
            tmp = subject.segfile.split('.mgz')[0] + f"_{os.environ['SLURM_ARRAY_TASK_ID']}" + '.mgz'
            LOGGER.info(tmp)
            print('FINISHED PRED')
            futures.append(
                eval.async_save_img(
                    tmp, pred_data, orig_img, dtype=np.int16
                )
            )
            print('FINISHED ASYNC SAVE IMG')
            ...
            store_aseg = subject.can_resolve_filename(f"{args.aseg_name}_test")
